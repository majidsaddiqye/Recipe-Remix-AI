...
exports.getRecipe = async (req, res) => {
  try {
    ...
    const aiResponse = await generateRecipeFromOpenAI(
      ingredients,
      cuisine,
      dietaryRestrictions
    );
  ...
  try {
    ...
    if (recipeData && !recipeId) {
      const newRecipe = await Recipe.create({
        ...
        ingredientsHash:
          recipeData.ingredientsHash || `chat-${Date.now()}-${req.user._id}`,
    ...
    if (!recipeIdToSave) {
      return res
        .status(400)
        .json({ message: "Recipe ID or Recipe Data is required" });
    }
    ...
    res
      .status(200)
      .json({ message: "Recipe saved successfully", recipeId: recipeIdToSave });
  } catch (error) {
...
exports.getSavedRecipes = async (req, res) => {
  try {
    ...
    // Filter out null values (recipes that verify deleted but still in user's list)
    const validSavedRecipes = user.savedRecipes.filter(
      (recipe) => recipe !== null
    );

    res.status(200).json({ data: validSavedRecipes });
  } catch (error) {
